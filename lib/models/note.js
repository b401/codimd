...
module.exports = function (sequelize, DataTypes) {
        ...
        return new Promise(function (resolve, reject) {
          ...
          if (!note.content) {
            ...
            if (!note.alias) {
              ...
              filePath = path.join(config.docsPath, note.alias + '.adoc')
            }
    ...
    async.series({
      parseNoteIdByAlias: function (_callback) {
        ...
        Note.findOne({
          ...
          if (note) {
            let filePath = path.join(config.docsPath, noteId + '.adoc')
            if (Note.checkFileExist(filePath)) {
            ...
            var filePath = path.join(config.docsPath, noteId + '.adoc')
            if (Note.checkFileExist(filePath)) {
              Note.create({
                alias: noteId,
                owner: null,
  ...
  Note.parseNoteInfo = function (body) {
    ...
    var $ = cheerio.load(adoc.convert(parsed.markdown,{ attributes: {showtitle: true}}))
    return {
  ...
  Note.generateWebTitle = function (title) {
    title = !title || title === 'Untitled' ? 'HackAdoc - Collaborative asciidoc notes' : title + ' - HackADoc'
    return title
  }
